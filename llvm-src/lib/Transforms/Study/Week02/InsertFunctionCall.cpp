...
namespace 
{
    ...
    //     }
    // }
    class InsertFunctionCall : public ModulePass
    {
    public:
        static char ID; // Pass identification
        InsertFunctionCall() : ModulePass(ID)
        {
            initializeInsertFunctionCallPass(*PassRegistry::getPassRegistry());
        }
...
ModulePass *llvm::createInsertFunctionCallPass()
{
    return new InsertFunctionCall();
}

INITIALIZE_PASS(InsertFunctionCall, "InsertFunctionCall",
                "InsertFunctionCall", false, false)

char InsertFunctionCall::ID = 0;
//static RegisterPass<llvm::Study::Week01::CopyPrintfCall> X("cpPrintf", "InsertFunctionCall Pass - created by LDH");
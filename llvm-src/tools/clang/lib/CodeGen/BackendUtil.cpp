...
#include "llvm/Transforms/Study/week02.h"
...
//LDH
static void addInsertFunctionCallPass(const PassManagerBuilder &Builder,
                                      legacy::PassManagerBase &PM)
{
  PM.add(createInsertFunctionCallPass());
}
static void addSummarizeFunctionCallPass(const PassManagerBuilder &Builder,
                                         legacy::PassManagerBase &PM)
{
  PM.add(createSummarizeFunctionCallPass());
}

static void addWrapFunctionPass(const PassManagerBuilder &Builder,
                                legacy::PassManagerBase &PM) {
    const PassManagerBuilderWrapper &BuilderWrapper =
            static_cast<const PassManagerBuilderWrapper &>(Builder);
    const CodeGenOptions &CGOpts = BuilderWrapper.getCGOpts();
    PM.add(createWrapFunctionPass(CGOpts.WrapFuncPrintMode));
}
...
void EmitAssemblyHelper::CreatePasses(legacy::PassManager &MPM,
  ...
  //LDH
  if (CodeGenOpts.EnableLDHOption)
  {
    PMBuilder.addExtension(PassManagerBuilder::EP_OptimizerLast,
                           addInsertFunctionCallPass);
    PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                           addInsertFunctionCallPass);
  }
  if (CodeGenOpts.EnableSummarizeFuncCallOption)
  {
    PMBuilder.addExtension(PassManagerBuilder::EP_OptimizerLast,
                           addSummarizeFunctionCallPass);
    PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,
                           addSummarizeFunctionCallPass);
  }

    if (CodeGenOpts.EnableWrapFunctionOption)
    {
        PMBuilder.addExtension(PassManagerBuilder::EP_OptimizerLast,
                               addWrapFunctionPass);
        PMBuilder.addExtension(PassManagerBuilder::EP_EnabledOnOptLevel0,

...
#undef CODEGEN_OPTION_WITH_MARSHALLING
  ...
  //LDH
  Opts.EnableLDHOption = Args.hasArg(OPT_fldh);
  Opts.EnableSummarizeFuncCallOption = Args.hasArg(OPT_fprofile_collect_func_info);
  Opts.EnableWrapFunctionOption = Args.hasArg(options::OPT_fwrap_func);
  if (Arg *A = Args.getLastArg(OPT_fwrap_func))
  {
    Opts.WrapFuncPrintMode = llvm::StringSwitch<bool>(A->getValue())
                                 .Case("1", true)
                                 .Case("0", false)
                                 .Default(false);
  }

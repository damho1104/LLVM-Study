...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  //LDH
  if (Args.hasArg(options::OPT_fldh))
  {
    CmdArgs.push_back("-fldh");
  }

  if(Args.hasArg(options::OPT_fprofile_collect_func_info))
  {
    CmdArgs.push_back("-fprofile-collect-func-info");
  }

  if(Arg *A = Args.getLastArg(options::OPT_fwrap_func)) {
      A->render(Args, CmdArgs);
  }
